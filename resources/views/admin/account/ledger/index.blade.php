    ...
    <section class="content">
        <div class="container-fluid">
            <div class="form-group row btn-warning">
                {{-- <div class="col-md-3 col-sm-3 float-right"><a href="{{ route('ledger.create') }}">
                        <h6>Add New ledger </h6>
                    </a></div>
                            ...
                            @if (isset($ledgers) && count($ledgers) > 0)
                                @foreach ($ledgers as $ledger)
                                    <tr>
                                        <td>{{ $n++ }}</td>
                                        {{-- <td>
                                                    <a href="{{ asset('uploads/ledgers/thumbnail/' . $ledger->image) }}"><img
                                                            src="{{ asset('uploads/ledgers/thumbnail/' . $ledger->image) }}"
                                                            height="80" width="150"></a>
                                                </td> --}}
                                        <td>{{ $ledger->ledger_name }}</td>
                                        <td>{{ $ledger->ledgergroup_name }}</td>
                                        <td>{{ $ledger->ledgergroup_code }}</td>


                                        {{-- <td><a href="{{ route('ledger.show', $ledger) }}">Show</a></td> --}}
                                        <td><a href="{{ route('ledger-group.edit', $ledger) }}">Edit</a></td>
                                        <td>
                                            ...
                                            {{-- <button type="submit" data-target="#exampleModal"
                                                            data-id={{ $ledger->id }} class="btn btn-danger delete"
                                            ...
                                            {{-- <a data-toggle="modal" id="deleteModal" data-target="#deleteModal"
                                                        data-attr="{{ route('ledger.destroy', $ledger->id) }}"
                                                        title="Delete ledger">
                                        ...
                                        <td>
                                            <a href="{{ route('ledger.destroy', $ledger) }}" data-id={{ $ledger->id }}
        ...
        <div class="modal-dialog" role="dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{ route('ledger.destroy') }}" method="post">
                        @csrf
                        @method('DELETE')
                        <input id="id" name="id" hidden>
                        <h5 class="text-center">Are you sure you want to delete this ledger?</h5>
                        {{-- <input id="firstName" name="firstName"><input id="lastName" name="lastName"> --}}
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-dismiss="modal">Cancel</button>
                    <button type="submit" class="btn btn-sm btn-danger">Yes, Delete ledger</button>
                </div>
                </form>
            </div>
        </div>
